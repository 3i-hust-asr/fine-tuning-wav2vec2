...
if __name__ == "__main__":
    ...
    print('load dataset ...')
    data_train = pd.read_csv(args.path_csv_train, encoding='utf-8')
    data_test = pd.read_csv(args.path_csv_test, encoding='utf-8')
    ...
    if args.add_noise:
        augment, _ = sf.read('data/noise.wav')
        train_data = train_data.map(augment_add_noise_speech_file_to_array_fn, num_proc=1)
    else:
        train_data = train_data.map(speech_file_to_array_fn, num_proc=1)
    test_data = test_data.map(speech_file_to_array_fn, num_proc=1)

    train_data = train_data.map(prepare_dataset, batch_size=args.batch_size, num_proc=1, batched=True)
    test_data = test_data.map(prepare_dataset, batch_size=args.batch_size, num_proc=1, batched=True)
